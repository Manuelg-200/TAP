...
public class MyLangTokenizer implements Tokenizer {
    ...
    private static enum Group {
        WHOLE_REG_EXP, SYMBOL, KEYWORD, SKIP, IDENT, NUM, VECTOR // WHOLE_REG_EXP unused, needed only for assigning the correct value to the other constants
    }
    ...
    private boolean boolValue; // the boolean value if the currently recognized tokean has type BOOL

    static { // initialization of the symbol and keyword tables: symbols and keyowrds are singleton lexical categories
    ...
    static {
        ...
        for (var s : symbols.keySet())
            symbolList.addFirst("\\" + String.join("\\", s.split(""))); // every char is pre-prended with "\\" to avoid regular expression syntax problems
        ...
        * remark: keywordRegEx uses word boundary '\b' since keywords match only if the next symbol is not a letter */
        ...
        /* builds the complete regular expressoin as union of the different groups
    ...
    private boolean hasNext() throws TokenizerException { // checks wether there are still lexemes
        ...
        while(true) { // reads the next non empty line, if any
            ...
            if (line.isEmpty()) // yep, lines can be empty!
                continue;
            matcher.reset(line); // reset the matcher with the new non null line
            return true;
        }
    }
    ...
    private TokenType assignTokenType() { // pre-condition: matcher.lookingAt() returned true
        ...
        if (matcher.group(Group.NUM.ordinal()) != null)
            return NUM;
        if (matcher.group(Group.VECTOR.ordinal()) != null)
            return VECTOR;
    ...
    @Override
    public void close() throws IOException { // tokenizers are auto-closable
        if (buf_reader != null)
            buf_reader.close();
    }
}
