...
/*** TO BE DONE 6.1 START ***/
	while(to_join[conn_no] != NULL) {
		...
		pthread_mutex_lock(&threads_mutex);
		if(no_response_threads[conn_no] > 1) {
			if(pthread_join(thread_ids[i], NULL) != 0)
				fail_errno("Could not join all threads");
			pthread_mutex_lock(&threads_mutex);
			...
			if (pthread_join(*to_join[conn_no], NULL) != 0)
				fail_errno("Could not join all threads");
...
/*** TO BE DONE 6.1 START ***/
	...
	if(pthread_join(thread_ids[i], NULL) != 0)
		fail_errno("Could not join prev thread");
	to_join[thrd_no] = to_join[i];
	to_join[i] = NULL;
	--no_response_threads[conn_no];
	connection_no[i] = FREE_SLOT;
	++no_free_threads;
...
/*** TO BE DONE 6.0 START ***/
	if (pthread_mutex_lock(&mime_mutex) != 0)
		fail_errno("Could not lock to mime mutex");
...
/*** TO BE DONE 6.0 START ***/
	if (pthread_mutex_unlock(&mime_mutex) != 0)
		fail_errno("Could not unlock to mime mutex");
