...
/*** TO BE DONE 6.0 START ***/
	if (pthread_mutex_lock(&cookie_mutex) != 0)
		fail_errno("Could not lock on cookie_mutex");
	...
	if (pthread_mutex_unlock(&cookie_mutex) != 0)
		fail_errno("Could not unlock on cookie_mutex");
...
/*** TO BE DONE 6.0 START ***/
	if ( gmtime_r(&now_t, &now_tm) == NULL)
		fail_errno("Could not convert response time to tm struct");
/*** TO BE DONE 6.0 END ***/
...
/*** TO BE DONE 6.0 START ***/
			if (fstat(fd, &stat_buffer) == -1)
				fail_errno("Could not get HTML_404 stats");
			file_size = stat_buffer.st_size;
			mime_type = get_mime_type(HTML_404);
			file_modification_time = stat_buffer.st_mtim.tv_sec;
...
/*** TO BE DONE 6.0 START ***/
			if(fstat(fd, stat_p) == -1)
				fail_errno("Could not get HTML_501 stats");
			file_size = stat_p->st_size;
...
/*** TO BE DONE 6.0 START ***/
			strcat(http_header, "\r\nSet-Cookie: ");
			sprintf(http_header+strlen(http_header), "%d=%d%s", cookie, UserTracker[cookie], COOKIE_EXPIRE);
...
/*** TO BE DONE 6.0 START ***/
		if (gmtime_r(&file_modification_time, &file_modification_tm) == NULL)
			fail_errno("Could not convert file modification time to tm struct");
...
/*** TO BE DONE 6.0 START ***/
		if (sendfile(client_fd, fd, NULL, file_size) == -1)
			fail_errno("Could not send file");
		close(fd);
		fd = -1; 
...
/*** METHOD POST FIX ***/
				if ( http_method == METHOD_POST) {
					option_val = strtok_r(NULL, "\n", &strtokr_save);
					if( strcmp(option_name, "Content-Length") == 0) {
						sscanf(option_val, "%d", &POST_content_length);
					}
				}
/*** METHOD POST FIX ***/
			}
		...
		if( http_method == METHOD_POST) {
			fgets(option_name, POST_content_length+1, client_stream);
		}
...
/*** TO BE OPTIONALLY CHANGED END ***/
		...
		if (access(filename, R_OK) != 0 || stat(filename, stat_p) < 0) {
			...
			if (MethodIsConditional(http_method)) {
				 ...
				 ***/
/*** TO BE DONE 6.0 START ***/
				if(stat_p->st_mtim.tv_sec <= my_timegm(&since_tm))
...
/*** TO BE OPTIONALLY DONE START ***/
				debug("	... receiving file %s\n", filename);
				SEND_RESPONSE(client_fd, RESPONSE_CODE_OK, UIDcookie,
#ifdef INCaPACHE_6_1
						is_http1_0, connection_no, thread_idx,
#endif
						filename, stat_p);
