	...
	for(int i=0; i<l->n_commands; i++) {
		if(i != l->n_commands-1) {
			if(l->commands[i]->out_pathname) {
				printf("Parsing error: redirection check failed\n");
				return CHECK_FAILED;
			}
		}
		if(i != 0) {
...
check_t check_cd(const line_t * const l)
{
	...
	for(int i = 0; i < l->n_commands; i++) {
		if(strcmp(CD, l->commands[i]->args[0]) == 0) {
			if(l->n_commands > 1) {
				printf("Parsing error: cd check failed\n");
				return CHECK_FAILED;
			}
	...
	while ((pid = wait(&wstatus)) > 0) {
		if(wstatus != 0) {
			if(WIFEXITED(wstatus))
				...
				printf("Child killed by signal, PID = %d, signal = %d\n", pid, WTERMSIG(wstatus));
			else if(WIFSTOPPED(wstatus))
				printf("Child stopped by signal, PID = %d, signal = %d\n", pid, WSTOPSIG(wstatus));
		}
	}
	/*** TO BE DONE END ***/
}
...
void redirect(int from_fd, int to_fd)
{
	...
	/*** TO BE DONE START ***/
		...
		if (dup2(from_fd, to_fd) == -1) 
			fatal_errno("Could not redirect file descriptor");		
		if( close(from_fd) == -1)
			fatal_errno("Could not close file descriptor in redirect");
	/*** TO BE DONE END ***/
}
...
void execute_line(const line_t * const l)
{
	...
	for(int a=0; a<l->n_commands; ++a) {
		...
		if (c->in_pathname) {
			...
			if(curr_stdin == -1) {
				perror("Could not open in_pathname file");
				return;
			}
			/*** TO BE DONE END ***/
		}
		if (c->out_pathname) {
			 ...
			 * (handling error cases) */
			/*** TO BE DONE START ***/
			curr_stdout = open(c->out_pathname, O_WRONLY | O_CREAT, S_IRWXU);
			if(curr_stdout == -1) {
				perror("Could not open out_pathname file");
				return;
			}
			/*** TO BE DONE END ***/
			...
			if (pipe2(fds, O_CLOEXEC) == -1)
				fatal_errno("Could not create a pipe");
			/*** TO BE DONE END ***/
			curr_stdout = fds[1];
			next_stdin = fds[0];
		}
...
int main()
{
	...
	for(;;) {
		...
		pwd = getcwd(NULL, 0);
		if(pwd == NULL)
			fatal_errno("Failed to get working directory");
		/*** TO BE DONE END ***/
