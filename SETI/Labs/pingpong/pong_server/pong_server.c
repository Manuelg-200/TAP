...
/*** TO BE DONE START ***/
		if( clock_gettime(CLOCK_TYPE, &time2) == -1)
			fail_errno("TCP Pong failed to get current time from system clock.");
...
/*** TO BE DONE START ***/
		if( clock_gettime(CLOCK_TYPE, &time2) == -1)
			fail_errno("UDP Pong failed to get current time from system clock.");
...
/*** TO BE DONE START ***/
		...
		if(udp_socket == -1)
			fail_errno("UDP Pong could not create socket");
...
/*** TO BE DONE START ***/
		if(request_socket == -1) {
			if(errno == EINTR)
				continue;
			close(server_socket);
			fail_errno("Pong server could not accept request");
		}
		pid = fork();
		if(pid == -1)
...
/*** TO BE DONE START ***/
	...
	if(gai_rv != 0) 
		fail_errno("Pong server could not setup server address");
	...
	if( bind(server_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) != 0)
		fail_errno("Pong server could not bind socket to address");
	if ( listen(server_socket, LISTENBACKLOG) != 0)
		fail_errno("Pong server could not mark socket with listen");
/*** TO BE DONE END ***/
