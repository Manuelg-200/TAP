...
/*** TO BE DONE START ***/
	if( clock_gettime(CLOCK_TYPE, &send_time) == -1)
		fail_errno("Failed to get current time from system clock.");
/*** TO BE DONE END ***/

    /*** Send the message through the socket (blocking)  ***/
/*** TO BE DONE START ***/
	sent_bytes = write(tcp_socket, message, msg_size);
	if(sent_bytes != msg_size)
		fail_errno("TCP Ping could not send message to server");
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	if (clock_gettime(CLOCK_TYPE, &recv_time) == -1)
		fail_errno("Failed to get the current time from system clock.");
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	if (getaddrinfo(argv[1], argv[2], &gai_hints, &server_addrinfo) != 0)
		fail_errno("Could not get a correct server address with the provided parameters.");
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	if(tcp_socket == -1)
		fail_errno("Failed to create a connection socket.");
	if (connect(tcp_socket, (struct sockaddr*)ipv4, server_addrinfo->ai_addrlen) != 0)
...
/*** TO BE DONE START ***/
	if (write(tcp_socket, request, strlen(request)) != strlen(request))
		fail_errno("Failed to send the request on the connection socket.");
/*** TO BE DONE END ***/
	nr = read(tcp_socket, answer, MAX_ANSW);
	if (nr < 0)
...
/*** TO BE DONE START ***/
	if(strcmp(answer, "ERROR\n") == 0)
		fail("Pong server answered with error message");
