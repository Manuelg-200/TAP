...
/*** TO BE DONE START ***/
	if( clock_gettime(CLOCK_TYPE, &send_time) == -1)
		fail_errno("UDP Ping could not get send time");
...
/*** TO BE DONE START ***/
	...
	if(sent_bytes == -1)
		fail_errno("UDP Ping could not send message to server");
...
/*** TO BE DONE START ***/
	if( clock_gettime(CLOCK_TYPE, &recv_time) == -1)
		fail_errno("UDP Ping could not get recv time");
...
/*** TO BE DONE START ***/
	if( fcntl(ping_socket, F_SETFL, O_NONBLOCK) == -1)
		fail_errno("UDP Ping could not set ping_socket to NONBLOCKing");
...
/*** TO BE DONE START ***/
	...
	if(gai_rv != 0)
		fail_errno("UDP Ping could not get server address info to create Ping Socket");
...
/*** TO BE DONE START ***/
	if( connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) == -1)
		fail_errno("UDP Ping could not connect UDP Socket to server");
...
/*** TO BE DONE START ***/
	...
	if(gai_rv != 0)
		fail_errno("UDP Ping failed to get server address to create ask Socket");
...
/*** TO BE DONE START ***/
	...
	if(ask_socket == -1)
		fail_errno("UDP Ping could not create request socket");
	if (connect(ask_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) == -1)
		fail_errno("UDP Ping could not connect to server");
/*** TO BE DONE END ***/
...
/** TO BE DONE START ***/
	if( write(ask_socket, request, strlen(request)) == -1)
		fail_errno("UDP Ping could not send request");
...
/*** TO BE DONE START ***/
	if(strcmp(answer, "ERROR\n") == 0)
		fail("Server sent back error message");
/*** TO BE DONE END ***/
